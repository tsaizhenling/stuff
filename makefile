...
# LIBS=-lm -lopencv_legacy -lopencv_highgui -lopencv_video -lgsl -lgslcblas -ljpeg

# Try using the following if you are on a mac:
#
 LIBS=-lm -L/opt/local/lib -lopencv_core -lopencv_legacy -lopencv_highgui -lopencv_video -lopencv_imgproc -lopencv_objdetect -lgsl -lgslcblas -ljpeg
#
 -lm:
 -L/opt/local/lib:
 -lopencv_core:
...
stasm: $(STASM_MAIN_OBJ) $(OBJ)
	$(LINK) -o stasm $(LFLAGS) $(STASM_MAIN_OBJ) $(LIBS)

minimal: $(MINIMAL_OBJ) $(OBJ)
